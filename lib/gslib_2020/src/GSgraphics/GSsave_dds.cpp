...
)
{
	...
	surface.dwFlags = DDSFile::FLAG_HEIGHT | DDSFile::FLAG_WIDTH | DDSFile::FLAG_CAPS | DDSFile::FLAG_PIXELFORMAT | DDSFile::FLAG_MIPMAPCOUNT;
	surface.ddsCaps.dwCaps1 |= DDSFile::CAPS1_TEXTURE;
	if (surface.dwMipMapCount >= 2) {
		surface.ddsCaps.dwCaps1 |= DDSFile::CAPS1_MIPMAP;
	}
	if (type == GL_TEXTURE_CUBE_MAP || type == GL_TEXTURE_3D) {
		surface.ddsCaps.dwCaps1 |= DDSFile::CAPS1_COMPLEX;
	}
	if (type == GL_TEXTURE_CUBE_MAP) {
		surface.ddsCaps.dwCaps2 |= DDSFile::CAPS2_CUBEMAP;
	} else if (type == GL_TEXTURE_3D) {
		surface.dwFlags |= DDSFile::FLAG_DEPTH;
