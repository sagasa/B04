...
void gsCleanUpLightmap(void);		/* ライトマップのクリーンアップ */
...
static GSboolean	s_bDefaultSkinMeshShader = GS_FALSE;
static GSboolean	s_bDefaultMeshShader     = GS_FALSE;
...
static GSuint		s_ExecuteDefaultSkinMeshShader = 0;
static GSuint		s_ExecuteDefaultMeshShader     = 0;
...
)
{
    ...
    s_bDefaultSkinMeshShader = GS_FALSE;
    s_bDefaultMeshShader     = GS_FALSE;

    s_ExecuteShader                = 0;
    s_ExecuteDefaultSkinMeshShader = 0;
    s_ExecuteDefaultMeshShader     = 0;
}
...
)
{
	...
	if (s_bSkinMesh == GS_TRUE && pSkeleton != NULL)
	{
        ...
        /* デフォルトのシェーダーの設定 */
        if (s_bDefaultSkinMeshShader == GS_TRUE) {
            gsBeginShader(s_ExecuteDefaultSkinMeshShader);
...
)
{
    ...
    if (pViewMatix == NULL)
    {
        glGetFloatv(GL_MODELVIEW_MATRIX, (GLfloat*)&ViewMatix);
    }
    else {
        ViewMatix = *pViewMatix;
    }
    /* デフォルトのシェーダーの設定 */
    if (s_bDefaultMeshShader == GS_TRUE) {
        gsBeginShader(s_ExecuteDefaultMeshShader);
...
/*=============================================================================
*
* Purpose : 指定アニメーションのキーフレームデータを取得．
*
* Return  : なし．
*
*============================================================================*/
extern void
gsGetAnimationKeyFrame
(
	GSuint			AnimationID,		/* アニメーションＩＤ			*/
	GSuint			AnimationNo,		/* アニメーションナンバ			*/
	GSuint			BoneID,				/* ボーンID						*/
	GSfloat			fTimer,				/* アニメーションタイマ			*/
	GSvector3*		pScale,				/*-> 拡大縮小キー				*/
	GSquaternion*	pRotate,			/*-> 回転キー					*/
	GSvector3*		pPosition			/*-> 平行移動キー				*/
)
{
	GSanimation*	pAnimation;
	...
	(
		pAnimation,
		AnimationNo, BoneID, fTimer,
		pScale, pRotate, pPosition
	);
}
...
)
{
    ...
    case GS_DEFAULT_MESH_SHADER:
        s_bDefaultMeshShader = GS_TRUE;
        break;
   
    case GS_DEFAULT_SKIN_MESH_SHADER:
        s_bDefaultSkinMeshShader = GS_TRUE;
        break;
	}
}
...
/*=============================================================================
*
* Purpose : デフォルトのメッシュシェーダーのバインド．
*
* Return  : なし．
*
*============================================================================*/
extern void
gsBindDefaultMeshShader
(
    GSuint ShaderID                         /* シェーダーID */
)
{
    s_ExecuteDefaultMeshShader = ShaderID;
}

/*=============================================================================
*
* Purpose : デフォルトメッシュシェーダーのバインド．
*
* Return  : なし．
*
*============================================================================*/
extern void
gsBindDefaultSkinMeshShader
(
    GSuint ShaderID                         /* シェーダーID */
)
{
    s_ExecuteDefaultSkinMeshShader = ShaderID;
}
...
)
{
    ...
    glGetLightfv(GL_LIGHT0, GL_DIFFUSE, (GLfloat*)&LightDiffuse);
    glGetLightfv(GL_LIGHT0, GL_SPECULAR, (GLfloat*)&LightSpecular);
    glGetLightfv(GL_LIGHT0, GL_POSITION, (GLfloat*)&LightPosition);
    glGetLightfv(GL_LIGHT0, GL_SPOT_EXPONENT, (GLfloat*)&LightSpotExponent);
    glGetLightfv(GL_LIGHT0, GL_SPOT_DIRECTION, (GLfloat*)&LightSpotDirection);
    glGetLightfv(GL_LIGHT0, GL_SPOT_CUTOFF, (GLfloat*)&LightSpotCutoff);
    glGetLightfv(GL_LIGHT0, GL_CONSTANT_ATTENUATION, (GLfloat*)&LightConstantAttenuation);
    glGetLightfv(GL_LIGHT0, GL_LINEAR_ATTENUATION, (GLfloat*)&LightLinearAttenuation);
    glGetLightfv(GL_LIGHT0, GL_QUADRATIC_ATTENUATION, (GLfloat*)&LightQuadraticAttenuation);
    /* グローバルアンビエントの取得 */
    glGetFloatv(GL_LIGHT_MODEL_AMBIENT, (GLfloat*)&LightModelAmbient);
    ...
    gsSetShaderParam4f("gs_LightDiffuse", &LightDiffuse);
    gsSetShaderParam4f("gs_LightSpecular", &LightSpecular);
    gsSetShaderParam4f("gs_LightPosition", &LightPosition);
    gsSetShaderParam3f("gs_LightSpotDirection", &LightSpotDirection);
    gsSetShaderParam1f("gs_LightSpotExponent", LightSpotExponent);;
    gsSetShaderParam1f("gs_LightSpotCutoff", LightSpotCutoff);
    gsSetShaderParam1f("gs_LightSpotCosCutoff", gsCos(LightSpotCutoff));
