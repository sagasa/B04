...
)
{
	...
	if ( pOctree->pNodes != NULL )
	{
		...
		{
			if ( pOctree->pNodes[i].pMesh != NULL )
			{
				/* 面データグループを削除 */
...
)
{
	...
	/* メッシュを持っていれば、メッシュとの衝突判定を行う */
	bCollision = GS_FALSE;
	if ( pNode->pMesh != NULL )
	{
		if (gsCollisionMeshAndLine(pNode->pMesh, pLine0, pLine1, pLine1, pPlane) == GS_TRUE)
		{
			bCollision = GS_TRUE;
		}
	}
...
)
{
	...
	if ( pNode->pMesh != NULL )
	{
		if (gsCollisionMeshAndSphere(pNode->pMesh, pCenter, Radius, pCenter) == GS_TRUE)
		{
			bCollision = GS_TRUE;
		}
	}
