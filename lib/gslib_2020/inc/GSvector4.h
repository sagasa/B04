...
GSINLINE GSvector4 GSvector4::lerp(const GSvector4& a, const GSvector4& b, float t) {
    return lerpUnclamped(a, b, CLAMP(t, 0.0f, 1.0f));
}

// 直線上にある 2 つのベクトル間を補間します
GSINLINE GSvector4 GSvector4::lerpUnclamped(const GSvector4& a, const GSvector4& b, float t) {
    return a + (b - a) * t;
}
